...
public class PlayerServiceImpl implements PlayerService {
    ...
    public List<Player> findByCriteria(final String firstName, final String lastName) {
        ...
        // First Name
        if (firstName != null) {
            javax.persistence.criteria.Predicate p = cb.equal(playerRoot.get(Player_.firstName), firstName);
            criteria = cb.and(criteria, p);
        }

        // Last Name
        if (lastName != null) {
            javax.persistence.criteria.Predicate p = cb.equal(playerRoot.get(Player_.lastName), lastName);
            criteria = cb.and(criteria, p);
        }
